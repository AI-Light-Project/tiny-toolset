...
**工具页右上角没有主题按钮？**
这是有意为之：主题统一在首页「齿轮 → 页面设置」里切换，工具页只跟随，避免悬浮按钮压住顶栏的操作按钮。
只有在既没有设置面板、也没有 `.topbar` 的页面上，`theme.js` 才会注入悬浮按钮兜底。
若首页的设置面板里也切不动主题，则是 `assets/js/theme.js` 加载失败——确认文件存在且没有被浏览器扩展拦截。
