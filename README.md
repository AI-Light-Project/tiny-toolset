...
**页面角落出现过星星/火箭/彩虹之类的小图案？**
那是早期版本 `theme.js` 注入的漂浮装饰插画，**已彻底移除**（左下角那张彩虹容易被误认成切换主题的按钮，
而且会压住卡片里的内容）。新版本 `theme.js` 不再往页面注入任何元素，并在 `apply()` 里顺手清掉历史残留，
所以强刷一次后就不会再出现。多巴胺主题的氛围由背景纹理（圆点 + 斜纹 + 光斑）与首页品牌插画提供。
