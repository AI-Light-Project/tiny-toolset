...
| **多巴胺** | 深紫黑底 + 五色系统（品红/青/黄/橙/紫），粗彩边框、堆叠硬阴影、圆点+斜纹+光斑三层图案、标题渐变字动画；**首页**另加漂浮卡通装饰（工具页不加，否则会压住卡片内容）。界面插画来自 Microsoft Fluent Emoji（MIT，见 `assets/img/CREDITS.md`） |
...
**工具页左下角有个彩虹/铅笔图案，点它没反应？**
那是多巴胺主题的漂浮装饰插画，不是按钮。它们现在**只在首页出现**——原先工具页也注入，会压在筛选列表等卡片内容上（左下角那张彩虹还容易被误认成切换主题的入口）。
